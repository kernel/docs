...
When you create a [Managed Auth connection](/auth/managed-auth), it is attached to a profile. A single profile can hold multiple auth connections, one per domain, so a browser launched with that profile loads the saved authentication state for each connection.
