...
Credentials let you store login information securely. Kernel can automatically authenticate when the login requires only stored credentials, or stored credentials and TOTP with a saved secret.
...
Common SSO provider domains (Google, Microsoft, Okta, Auth0, GitHub, etc.) are allowed by default, so you don't need to add them to `allowed_domains`. If your login has additional verification or approval requirements, [contact support](/info/support) to find the best configuration:
