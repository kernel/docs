...
Common SSO provider domains (Google, Microsoft, Okta, Auth0, GitHub, etc.) are allowed by default, so you don't need to add them to `allowed_domains`. If your login has additional input, choice, or approval requirements, [contact support](/info/support) to find the best configuration:
