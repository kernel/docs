...
By default, Kernel saves durable credential fields after a successful login. Kernel can automatically reauthenticate credential-only flows and attempts to provide TOTP codes when needed. Submitted one-time codes aren't saved.
...
Setting `auto_reauth: true` permits Kernel to attempt automatic reauthentication. The connection must also have a credential that covers the login flow. If your flow has additional input, choice, or approval requirements, [contact support](/info/support) to find the best configuration.
