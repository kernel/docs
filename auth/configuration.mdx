...
By default, Kernel saves durable credential fields after a successful login. Automatic reauthentication is supported when the login requires only stored credentials, or stored credentials and TOTP with a saved secret. Submitted one-time codes aren't saved.
...
Setting `auto_reauth: true` permits Kernel to attempt automatic reauthentication. The connection must also have a credential that covers the login flow and a stored TOTP secret when the site requires one. If your flow has additional verification or approval requirements, [contact support](/info/support) to find the best configuration.
