...
Managed Auth supports common credential, SSO, and multi-step login flows. Automatic reauthentication uses stored credentials and attempts to provide TOTP codes when needed. Authentication requirements vary by site, so [contact support](/info/support) if your flow uses additional input, choices, or approval steps.

<Warning>
Passkey-only authentication isn't currently supported. If a site's SSO provider requires a passkey, the login returns `unsupported_auth_method`. [Contact support](/info/support) to find the best configuration for that flow.
</Warning>
