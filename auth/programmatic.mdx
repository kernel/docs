...
A successful interactive login can save submitted credentials for automatic reauthentication. The supported paths require only stored credentials, or stored credentials and TOTP with a saved secret. If your login has additional requirements, [contact support](/info/support) to find the best configuration.
