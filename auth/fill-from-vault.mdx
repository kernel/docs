...
you can collect values from an end user with KERNEL's hosted collection form or [copy them from an existing credential vault](/vaults/existing-credential-vault) that your trusted backend can read. if your users already log in through Managed Auth, [reuse that saved credential](/auth/fill-from-managed-auth) instead of collecting it again. each path produces a ready credential item and uses the same `fill` operation.
