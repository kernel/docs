...
A Managed Auth Connection associates a [profile](/auth/profiles) to a domain you want to keep authenticated so you can use the auth connection future browsers.
...
Common SSO provider domains (Google, Microsoft, Okta, Auth0, GitHub, and more) are automatically allowed. For custom or uncommon OAuth providers, add their domains to `allowed_domains`:
...
Route login flows through a [proxy](/proxies/overview) to avoid bot detection on specific sites. Set the proxy when creating a connection or override it per-login:

<CodeGroup>
```typescript TypeScript
// Set proxy on the connection
const auth = await kernel.auth.connections.create({
  domain: 'example.com',
  profile_name: 'my-profile',
...
// Or override proxy for a specific login
const login = await kernel.auth.connections.login(auth.id, {
...
The login response includes a `live_view_url` you can use to watch the login flow in real time. This is useful for debugging failed logins:
