...
<Info>
Automatic reauthentication is supported when the login requires only stored credentials, or stored credentials and TOTP with a saved secret. If your login has additional requirements, [contact support](/info/support) to find the best configuration.
</Info>
