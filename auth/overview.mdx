...
[Profiles](/browsers/profiles) persist cookies and local storage between browser sessions. Managed Auth saves successful logins to a profile automatically. A workflow using Vaults + Credential Fill can also save the resulting browser state to a profile when it needs to reuse that session.
