...
description: "Compare KERNEL-hosted credential collection with 1Password brokered approval, then connect an account, request access, and fill an approved login"
---
...
{/* TODO(1password-sdk-preview): replace this note with the first published SDK release that includes 1Password vault items. Pins come from the stlc build for kernel/kernel@dd27dc7. */}

<Note>
  **temporary:** the code samples on this page were typechecked against an
  unreleased development SDK build generated from the api change:
  `@onkernel/sdk` at `kernel-node-sdk-staging@bc922de` and `kernel` for python
  at `kernel-python-sdk-staging@ff3e97a`. no published SDK release includes
  these types yet. this note will name the release once it ships.
</Note>
