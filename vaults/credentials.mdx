...
credential items store values and support explicit [browser fill](/vaults/fill). your application or agent still handles navigation, submission, and the site's response, so you keep control over the auth experience and visibility into each step. choose [managed auth](/auth/overview) instead when you want KERNEL to run and maintain login flows for you.
