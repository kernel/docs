...
{/* TODO(credential-provider): `provider: "kernel"` is required by the api change in kernel/kernel@dd27dc7; create requests without it return 400. samples were typechecked against the unreleased stlc sdk preview for that commit (node kernel-node-sdk-staging@bc922de, python kernel-python-sdk-staging@ff3e97a). re-check against the first published sdk release. updates don't accept `provider`. */}
