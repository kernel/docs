...
a 1Password-backed credential item is in development. it uses a separate
brokered-approval flow rather than copying values; see [1Password
credentials](/vaults/1password) for how it compares and its current
availability.
...
- [1Password credentials](/vaults/1password): compare KERNEL-hosted collection with 1Password brokered approval.
