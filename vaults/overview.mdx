...
selected browser inputs for credential items and eligible link cards, and returns value-free outcomes. agentcard aliases are
non-sensitive stand-ins that KERNEL handles at egress, outside the browser.
...
return masks; only agentcard cards return aliases. credential fields marked `sensitive: false` return
...
ready credential items and eligible ready, unexpired link cards can advertise `fill`. your
controller supplies field names and selectors; KERNEL writes the actual values into
the attached browser. link cards additionally require an exact current top-level https `page_url` at the card's `merchant_url` origin. credential items have separate page-selection rules and no destination allowlist. see [fill browser fields](/vaults/fill).

fill doesn't explicitly submit a form or confirm the site's acceptance. preserve its per-field outcomes, and don't automatically retry failed or unknown attempts. link cards don't expose aliases or support egress substitution.

agentcard cards publish non-sensitive, format-valid aliases: a
...
session and vault are bound together. agentcard does not use fill.
...
### Agentcard alias handoff happens at egress

the <span className="kernel-brand-name">KERNEL</span> egress layer runs outside
the browser vm. when it recognizes an agentcard request containing aliases, it verifies
the browser, session, project, vault, item, and provider state. KERNEL holds the request for approval, agentcard executes it, and KERNEL replays the response to the browser without exposing the underlying card. handoff fails closed when a binding or state check does not match.
...
| alias              | non-sensitive, format-valid stand-in returned in agentcard card state; aliases belong to that item |
...
a wallet connects an end user's payment method through a provider-hosted flow. the card's provider determines how an attached browser uses it:

- **link:** approve a purchase and wait for a ready, unexpired card with stored encrypted material. invoke its advertised `fill` operation to write actual values into checkout fields. the exact current top-level https page must share the origin of `spec.merchant_url`. fill does not consume the item or clear its encrypted card material; single-use is a property of the provider-issued card, not an alias lifecycle.
- **agentcard:** use the reusable card item's aliases. KERNEL intercepts a recognized processor request from the attached browser, holds it for approval, and replays agentcard's response. every checkout requires approval.

link and agentcard are credential providers, not merchant payment processors. link fill does not require an adapter. agentcard's alias handoff requires a native processor adapter; see [checkout and processor coverage](/integrations/payments/overview#checkout-and-processor-coverage).

both paths require the vault attachment and applicable item lifecycle checks. wallet connection, authorization, fill attempts, and provider handoff have value-free audit events. neither a completed fill nor an approved handoff proves payment success. stop and reconcile failed or unknown outcomes without automatically retrying or switching to aliases.
...
deleting a card invalidates any agentcard aliases and clears any stored provider value.
...
card state can include `masks.brand` and `masks.last4`. only agentcard cards expose read-only `aliases`: `number`, `cvc`, `exp_month`, and `exp_year`. aliases are non-sensitive stand-ins, not standalone credentials or permission to use the provider-backed value. link cards advertise `fill` when eligible instead.

## Next steps

- [credential items](/vaults/credentials): define fields, collect values, and update them safely.
- [fill browser fields](/vaults/fill): map credential or link card fields to browser inputs and handle outcomes.
...
- [enable payments in a browser agent](/browsers/enable-payments-in-browser-agent): connect a wallet and use link fill or agentcard aliases for checkout.
