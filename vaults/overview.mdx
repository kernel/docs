...
selected browser inputs for credential items and returns value-free outcomes.
...
a wallet connects an end user's payment method through a provider-hosted flow. the card's wallet determines how an attached browser uses it:

- **link:** approve a purchase and wait for a ready, unexpired, single-use card with stored encrypted material. invoke its advertised `fill` operation to write values into checkout fields. the exact current top-level https page must share the origin of `spec.merchant_url`.
...
link and agentcard are credential providers, not merchant payment processors. link does not require an adapter. agentcard's alias handoff requires a native processor adapter; see [checkout and processor coverage](/integrations/payments/overview#checkout-and-processor-coverage).
...
card state can include `masks.brand` and `masks.last4`. only agentcard cards expose read-only `aliases`: `number`, `cvc`, `exp_month`, and `exp_year`. aliases are non-sensitive stand-ins, not standalone credentials or permission to use the provider-backed value.
...
- [enable payments in a browser agent](/browsers/enable-payments-in-browser-agent): connect a wallet and complete browser checkouts.
