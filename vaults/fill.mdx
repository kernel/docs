...
description: "fill credential and link card fields in an attached browser with value-free api outcomes"
---
...
agentcard cards do not support fill; they retain their [alias-based checkout flow](/integrations/payments/agentcard). link cards do not expose `state.aliases` or support egress substitution.
...
**credentials:** omit `page_url` only when the browser has exactly one open page. credential items do not inherit link's https or merchant-origin restrictions and have no destination allowlist. your trusted controller must authorize the destination before disclosing credentials; neither `description` nor `page_url` grants or restricts that permission.
...
**don't automatically retry fill after a failure or uncertain outcome.** a lost response can follow successful writes; another request can repeat events, overwrite edits, or generate a different totp code. deliberate recovery starts with inspecting the existing attempt, not replaying it. link has no alias fallback. preserve per-field `index`, `status`, and any `error_code` for reconciliation.
