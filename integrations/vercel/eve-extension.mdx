...
```bash
vercel connect create kernel --name eve-extension
vercel connect attach kernel/eve-extension
```

`kernel` is Kernel's entry in Vercel's connector registry, so Vercel fills in the MCP URL, auth type, and branding, then opens your browser for the authorization step. You can also add it from the Vercel dashboard under **Connectors → Browse all → Kernel**. Confirm the UID with `vercel connect list`.
...
agent/extensions/kernel/
  extension.ts             # export default kernel({ connect: "kernel/eve-extension" })
