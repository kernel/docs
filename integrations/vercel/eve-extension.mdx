...
- **`screenshot`**: capture what the session currently displays, so the agent can see the page.
- **`computer_action`**: human-like mouse, keyboard, and clipboard input for the same session, for surfaces Playwright selectors can't reach.
