...
<Info>
Kernel returns an authenticated `webdriver_ws_url` for each browser session, so you can pass that URL directly to Vibium. You do not need to add custom authorization headers or set extra Vibium auth environment variables for the examples below.
</Info>
...
```bash CLI
# Install Vibium for the CLI and MCP examples
