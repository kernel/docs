...
| square                         | prepared single-use checkout (currently enabled) | JSON card-nonce requests to `pci-connect.squareup.com/v2/card-nonce` and `pci-connect.squareupsandbox.com/v2/card-nonce`                                                                    |
...
Mercado Pago don't support the ordinary hold flow. The current KERNEL deployment
has prepared checkout enabled, so new Square checkouts must use
