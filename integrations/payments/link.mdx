...
after the user completes the approval action, retrieve the card with `wait: 60` until it becomes `ready`. pass `state.aliases` to the [browser checkout guide](/browsers/pay-through-browser-checkout).
