...
exposure and may help limit pci dss scope. your exact scope depends on your
integration and environment; confirm it with your compliance advisor or qsa.
...
existing item. when both link by stripe and agentcard wallets exist, show both as
...
<CardGroup cols={2}>
  <Card
    ...
    icon="link"
  >
    collect a link by stripe wallet and approve a one-use credential for a specific
    purchase.
  </Card>
  <Card
    title="Agentcard"
...
<Note>
  link by stripe and agentcard identify where the credential comes from and how the
...
for example, a browser can enter link by stripe or agentcard aliases into a Shopify
checkout. Shopify remains the merchant platform; link by stripe or agentcard
