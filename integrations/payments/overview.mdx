...
<Note>
  this guide describes alias-based payments. [direct fill](/vaults/fill) is also
  available for eligible link by stripe cards, but writes real values into the browser and
  has a different exposure boundary. credentials for non-payment forms belong in
  [credential items](/vaults/credentials), not wallets or cards.
</Note>
