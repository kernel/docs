---
title: "overview"
description: "connect wallets and approve browser payments with link fill or agentcard aliases"
---
...
- **link:** an approved, provider-issued single-use card advertises [fill](/vaults/fill). KERNEL writes actual card values into an attached browser without returning them in the api response or explicitly submitting checkout. link no longer exposes aliases or supports egress substitution.
...
link and agentcard are credential providers, not merchant payment processors. the merchant's processor doesn't need to be stripe. link fill depends on supported checkout inputs and the allowed merchant origin; agentcard handoff requires a native processor adapter.

<Warning>
  link fill exposes real card values to the browser, including page scripts, extensions, and agents with browser or cdp access. don't assume those values are excluded from recordings. the vault api's value-free responses do not guarantee browser isolation or any particular pci dss scope; assess your own integration's data handling and compliance obligations.
</Warning>
...
flowchart LR
  ...
  L --> F[fill actual values into attached browser]
...
| payment handoff           | actual card values filled into an attached browser      | agentcard executes the intercepted request; KERNEL replays the response |
| reuse                     | provider-issued single-use card; fill does not consume the item | reusable card item; each checkout requires approval             |
| environment               | live only                                                | credential-defined; customer-owned configs expose `test_mode`       |

choose [link by stripe](/integrations/payments/stripe-link) when each purchase requires a newly approved, single-use credential. choose [agentcard](/integrations/payments/agentcard) when one enrolled card must support multiple purchases, with separate approval for each.
...
<Warning>
  ...
  error, missing response, completed link fill, or reusable agentcard item does
...
configure [link by stripe](/integrations/payments/stripe-link) or [agentcard](/integrations/payments/agentcard), then follow [enable payments in a browser agent](/browsers/enable-payments-in-browser-agent) to attach the vault and use the provider's checkout path.
