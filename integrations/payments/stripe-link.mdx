...
[link by stripe](https://stripe.com/payments/link) connects a user's wallet through oauth and issues a single-use card for an approved purchase. <span className="kernel-brand-name">KERNEL</span> stores the card material with kms-backed envelope encryption. an eligible card item advertises [fill](/vaults/fill), which writes its actual values into an attached browser without returning them in the api response or explicitly submitting checkout. link does not expose `state.aliases` or substitute card values at egress.

link is the credential provider, not the merchant's payment processor. fill does not require a native processor adapter; it requires supported, uniquely selectable checkout inputs on an allowed merchant page. the merchant's processor doesn't need to be stripe. filling fields does not establish that the processor accepts the card or that the purchase succeeded.

<Warning>
  fill puts real card values in the browser. page scripts, extensions, and an agent with browser or cdp access may read them. don't assume recordings or other browser observation surfaces exclude those values.
</Warning>
...
`merchant_url` supplies provider context and restricts fill to its https origin. the request's `page_url` must exactly match one current top-level page, including path, query, and fragment, and share that origin. the host and effective port must match the `merchant_url` origin; the default https port `443` is normalized. use an https `merchant_url` without embedded credentials for fill; `state.domains` is metadata, not the fill authorization rule.

## Fill the checkout

after the user completes approval, retrieve the card with `wait: 60`. continue only when it is `ready` and advertises `fill`; one bounded wait may finish before approval does. follow the [typescript, python, and cli fill examples](/vaults/fill#fill-link-card-fields) or the [browser agent payments guide](/browsers/enable-payments-in-browser-agent#4-fill-link-cards-or-pass-agentcard-aliases).
...
don't repeat `authorize` or create a replacement item to retry an unknown purchase. inspect existing fill outcomes, item events, and the merchant's order state first.
