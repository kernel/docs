...
link by stripe is the credential provider, not the merchant's payment processor. at
...
example, you can use a link by stripe credential in a Shopify checkout.
...
before showing a link by stripe connection option, list the vault's items. if a
