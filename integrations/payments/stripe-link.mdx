...
`connected` wallet; continue with payment-method selection. client credentials
alone do not connect a user. config registration does not verify the link
client secret; import verifies access to payment methods, not refresh-token
validity. KERNEL owns refresh-token rotation after import, so your backend must
stop refreshing that grant.
