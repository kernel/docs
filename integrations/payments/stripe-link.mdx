...
use KERNEL's oauth client unless you need to own the integration. the default
flow below lets KERNEL construct the authorization url, handle the redirect,
and manage the user's oauth grant.
...
<Accordion title="use your own oauth client (optional)">

bringing your own client means implementing the user authorization flow in your
trusted backend instead of using KERNEL's `link_oauth` action:

1. construct the authorization url with state and a pkce challenge, and show it only to the authenticated human connecting their link wallet.
2. register your backend's redirect uri with the provider, not KERNEL's callback. validate the returned state against that user's authorization attempt.
3. exchange the authorization code with stripe from your backend, using the original pkce verifier and your client credentials. never expose the client secret, verifier, or returned tokens to the agent, browser code, urls, or logs.
4. create the KERNEL wallet item with the resulting access and refresh tokens from that same grant, under your registered provider configuration.

register the application credentials once using organization-scoped
authentication. the protected json file must contain `client_id` and
`client_secret`; do not put either value in shell arguments.
...
then import the user's grant from your trusted backend or human-operated
terminal. `link-grant.json` must contain `access_token` and `refresh_token` from
that grant, with a currently valid access token. use files readable only by
their owner (for example, mode `0600`), outside the agent's accessible files.
both file flags also accept `-` for stdin from a trusted secret source.
...
this replaces the wallet-connection step below. a successful import returns a
`connected` wallet; continue with payment-method selection.

- **registration:** client credentials alone do not connect a user. config registration does not verify the link client secret.
- **import:** KERNEL verifies access to payment methods, not refresh-token validity.
- **rotation:** KERNEL owns refresh-token rotation after import. your backend must stop refreshing that grant.

repeating wallet creation never replaces its grant. if an imported wallet
becomes unusable, obtain a fresh grant and import it under a new wallet key for
future purchases. retain old items with uncertain payments; do not replay those
payments or rebind their cards. use an explicit cutover rather than the default
one-wallet setup below. see [provider configurations](/vaults#provider-configurations)
for binding, rotation, and deletion rules.
