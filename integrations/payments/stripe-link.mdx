...
[link by stripe](https://stripe.com/payments/link) connects a user's wallet through oauth and issues a single-use card for an approved purchase. <span className="kernel-brand-name">KERNEL</span> stores the card material with kms-backed envelope encryption. an eligible card item advertises [fill](/vaults/fill), which writes its values into an attached browser without exposing the details to the agent or model, and does not return the item in the api response.
...
after the user completes approval, retrieve the card with `wait: 60`. continue only when it is `ready` and advertises `fill`; one bounded wait may finish before approval does. the examples below show the fill request; for the full controller and agent handoff, follow the [browser agent payments guide](/browsers/enable-payments-in-browser-agent#4-fill-link-cards-or-pass-agentcard-aliases).
