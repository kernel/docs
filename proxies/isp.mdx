...
<Warning>
ISP proxies don't get automatic `.gov` routing. Kernel attaches the provider that handles `.gov` to residential proxies only, so `.gov` requests from an ISP proxy fall back to the default provider, which refuses some destinations and returns a `502`.
</Warning>
