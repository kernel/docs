...
ISP proxies don't route restricted destinations automatically. Kernel attaches the provider that carries `.gov`, country government domains, and banking and payment destinations to residential proxies only, so those requests from an ISP proxy fall back to the default provider, which carries some of them and returns a `502` on the rest.
