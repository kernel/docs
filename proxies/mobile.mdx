...
Mobile proxies don't route restricted destinations automatically. Kernel attaches the provider that carries `.gov`, country government domains, and banking and payment destinations to residential proxies only, so those requests from a mobile proxy fall back to the default provider, which carries some of them and returns a `502` on the rest.

If your workload targets them, use a [residential proxy](/proxies/residential#restricted-destination-routing) with no targeting, country-only targeting, or US state targeting.
