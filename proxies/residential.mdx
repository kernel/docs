...
Residential proxies created on or after September 9, 2026 with no targeting, country-only targeting, or US state targeting automatically route `.gov` requests through a compatible upstream provider. ZIP, ASN, non-US state, and older proxies may return `restricted_route_unavailable`.

You may [check proxy health](/proxies/overview#check-proxy-health) against a `.gov` URL before starting a browser.
