...
Residential proxies created on or after September 9, 2026 with no targeting, country-only targeting, or US state targeting automatically route `.gov` requests through a compatible upstream provider. City-targeted proxies support `.gov` when Kernel validates the route at creation. ZIP, ASN, non-US state, and older proxies may return `restricted_route_unavailable`. Other traffic continues through the standard residential route.

You can [check proxy health](/proxies/overview#check-proxy-health) against a `.gov` URL before starting a browser.
