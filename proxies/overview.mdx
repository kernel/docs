...
<Warning>
The browser's egress network is momentarily disconnected during a proxy hot swap. Any in-flight requests may fail and need to be retried.
</Warning>
...
<Note>
If you swap the proxy on a browser acquired from a pool, the browser will be baselined back to the pool's default proxy configuration when it is released back to the pool. This means releasing the browser will be delayed by the swap duration (~2-3 seconds) while the proxy is restored to the pool default.
</Note>
