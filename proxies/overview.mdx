...
<Note>
Kernel-provided proxies are unmetered and not billed, subject to the fair use rules and restricted destinations in our [Acceptable Use Policy](/acceptable-use#proxy-fair-use). New residential proxies with no targeting, country-only targeting, or US state targeting automatically route `.gov` requests through a compatible upstream provider. City targeting works when Kernel validates the route during proxy creation; ZIP, ASN, non-US state, and older proxy configurations may return `restricted_route_unavailable`. Other restricted destination categories, including banking and payment domains, may still require direct access or a [custom proxy](/proxies/custom).
</Note>
