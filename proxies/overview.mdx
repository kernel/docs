...
[Stealth browsers](/browsers/bot-detection/stealth) are automatically assigned a residential proxy. To disable this on a running stealth browser and route traffic directly, set `disable_default_proxy` to `true`:

<CodeGroup>
```typescript Typescript/Javascript
// Disable the default stealth proxy
await kernel.browsers.update(browser.session_id, {
  disable_default_proxy: true,
});

// Re-enable the default stealth proxy
await kernel.browsers.update(browser.session_id, {
