...
<Note>
Kernel-provided proxies are unmetered and not billed, subject to the fair use rules and restricted destinations in our [Acceptable Use Policy](/acceptable-use#proxy-fair-use). Some destination categories — including government, banking, and payment domains — are blocked by our upstream network providers. [Residential proxies](/proxies/residential#restricted-destination-routing) route many of these automatically; for a destination outside that set, use a [custom proxy](/proxies/custom) or [request an exception](/acceptable-use#requesting-an-exception).
</Note>
