...
| `restricted_route_unavailable` | The destination requires a specialized egress route and Kernel could not build one. Kernel will not fall back to a provider known to reject the destination. | Yes |
...
Proxy failures are also reported as `proxy_error` [browser telemetry](/browsers/telemetry/overview) events in the `network` category, carrying the same code. Use the header to recover in the moment, and telemetry to attribute failures per session and per URL afterwards.
