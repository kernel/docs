...
| `restricted_route_unavailable` | The destination requires a specialized egress route, the proxy is configured to carry one, and Kernel could not build it. Kernel will not fall back to a provider known to reject the destination. | Yes — the route is resolved again on a later attempt. If it persists, create a new residential proxy with no targeting, country-only targeting, or a U.S. state, confirm it with [`POST /proxies/{id}/check`](https://kernel.sh/docs/api-reference/proxies/check-proxy-health) against the destination URL, then use it for the session. |
...
| `provider_blacklisted` | The upstream proxy provider blocks this destination. A residential proxy with ZIP, ASN, or non-US state targeting returns this rather than `restricted_route_unavailable`, because [automatic routing](/proxies/residential#restricted-destination-routing) is never attached to those configurations. | No. A [residential proxy](/proxies/residential#restricted-destination-routing) with supported targeting carries many of these destinations; otherwise use a [custom proxy](/proxies/custom) or [request an exception](/acceptable-use#requesting-an-exception). |
