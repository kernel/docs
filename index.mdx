...
[browser pools](/browsers/pools/overview) keep browsers warm and pre-configured so you skip start-up latency on every task. idle browsers in a pool aren't billed, so most production workloads run on them from day one.