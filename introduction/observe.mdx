...
If you're running an agent on Kernel's [app platform](/apps/overview), every invocation produces a streaming log feed. Tail it live while the agent runs, or pull it after the fact for debugging.
...
Full reference: [Logs](/apps/build-deploy-operate#logs).
...
- **Instrumenting the agent itself?** Drop [screenshots](/browsers/computer-controls#take-screenshots) and [logs](/apps/build-deploy-operate#logs) into your traces at the points that matter.
