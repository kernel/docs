...
For the mechanics of standing up a browser pool and acquiring browsers from it, see the [Browser Pools overview](/browsers/pools/overview). This guide covers which architecture to wrap around it.
...
- High (100+): Use multiple browser pools with queuing

**Request Patterns**: How do tasks arrive?
- Steady state: Size your browser pool to match average load
- Bursty traffic: Size for burst capacity or implement queuing
- Scheduled batches: Pre-configure browser pools to match batch processing size
...
**When to use:**
- Request volume exceeds maximum browser pool capacity (100+ concurrent)
