...
Kernel browsers expose four ways to drive a session. For agents, we recommend starting with [playwright execution](/browsers/playwright-execution) and falling back to [computer use](/browsers/computer-controls) for the interactions it can't reliably drive — see our [cookbook](/browsers/playwright-computer-use-fallback) for the pattern. Both run co-located with the browser and avoid the bot-detection surface a direct CDP connection introduces.
