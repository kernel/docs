...
This is a known server-side issue — see [Troubleshooting](/reference/mcp-server/troubleshooting#oauth-issuer-mismatch-on-strict-clients) — not something to fix client-side. Until it's resolved, [connect with an API key](#connect-with-an-api-key-workaround) instead of OAuth.
</Warning>
