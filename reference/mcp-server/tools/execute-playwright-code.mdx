...
An invocation can return `completed`, `canceled`, `error`, or `awaiting_submission`. The last status means a non-autosubmit declarative form was populated but not submitted. Inspect the form, obtain any required confirmation, then submit it through Playwright or computer interaction without invoking the tool again. See [handling a populated form](/browsers/webmcp#handle-a-populated-form).
