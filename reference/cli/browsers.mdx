...
| Flag | Description |
|------|-------------|
| `--categories <list>` | Comma-separated categories to include (`console`, `network`, `page`, `interaction`, `system`, `api`). Invalid categories are rejected. |
| `--types <list>` | Comma-separated event types to include (e.g. `network_request,console_error`). |
| `--seq <n>` | Resume from this sequence number (must be >= 1). Omit to stream from now. |
