...
but not the underlying value. these response fields are not general-purpose
secret storage. do not put card numbers, cvc, passwords, api keys, oauth codes,
or other secrets in item keys, specifications, metadata, merchant fields,
context, or events.

for link by stripe, KERNEL stores oauth credentials and approved one-use card
...
link by stripe creates a one-use card for an approved purchase. agentcard keeps a
