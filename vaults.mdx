...
<Note>
  ...
  credential items don't require a provider or wallet.
</Note>
...
their populated values in `state.fields`; explicitly use that setting for
ordinary usernames and email addresses.
...
ready credential items and eligible link cards can advertise `fill`. your
...
the payment lifecycle below describes aliases. for direct filling of eligible
link cards, see [fill browser fields](/vaults/fill).
...
- [fill browser fields](/vaults/fill): map credential or supported card fields to browser inputs and handle outcomes.
- [use vault credentials in a browser agent](/browsers/use-vault-credentials-in-browser-agent): connect trusted collection to a scoped agent tool.
