...
With [browser pools](/browsers/pools), you pay the standard usage-based price per GB-second while browsers are running. Idle browsers in a browser pool incur no disk charges — you only pay when a browser is actively in use, so keeping a browser pool warm between tasks costs nothing.
...
Kernel enforces a single concurrency limit covering all browsers you run at once — whether created on demand with `browsers.create()` or warmed in a [browser pool](/browsers/pools). Your full limit is available to either API in any mix.
