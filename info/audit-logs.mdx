...
| [One-time export](#export-audit-logs) | Archival, compliance, and offline analysis of a bounded time window | Gzip-compressed JSON Lines (`.jsonl.gz`) |
...
Search and one-time export use an inclusive `start` and exclusive `end`: `[start, end)`. Each request can cover up to 30 days. Split longer periods into multiple time windows. Continuous S3 export starts delivering when you activate a destination and does not backfill earlier events.

Search, one-time export, and continuous export destinations are also available from the [CLI](/reference/cli/audit-logs). For the underlying HTTP API for search and one-time export, see [search](https://kernel.sh/docs/api-reference/audit-logs/list-audit-logs) and [export](https://kernel.sh/docs/api-reference/audit-logs/download-an-audit-log-export-chunk) in the API reference.

## Filter audit logs

The API and SDKs use the same filters for search and one-time export:
...
This is a one-time export for a bounded time window. The SDK download helpers default to `jsonl.gz` and write a complete export to a destination you provide. They:
...
### Choose one-time or continuous export

| If you need to... | Use |
|-------------------|-----|
| Investigate recent requests interactively | Search audit logs |
| Produce a bounded file for an archive or an offline job | One-time SDK or HTTP export |
