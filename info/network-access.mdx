...
The destinations and ports on this page do not change without a [changelog](/changelog) entry and advance notice. If Kernel needs to serve traffic from somewhere new, it appears here before it reaches your account.

Within the scope of this page, the following are deliberately not covered and can change at any time:

- The specific hostname inside a wildcard, as returned in `cdp_ws_url`, `webdriver_ws_url` or `browser_live_view_url`. It changes between sessions.
- Which metro or host serves a given session.
- Egress IP addresses, including residential, ISP and datacenter proxy exit IPs.
