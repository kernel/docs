...
- **SSO/OAuth support** - "Sign in with Google/GitHub/Microsoft" buttons work out-of-the-box. Common SSO domains (Google, Microsoft, Okta, Auth0, GitHub, and more) are [allowed by default](#default-sso-domains)
...
- **Proxy support** - Route authentication through proxies for geo-targeting or IP management
- **Secure by default** - Credentials encrypted at rest, never exposed in API responses, or passed to LLMs

## Default SSO domains

Common SSO/OAuth provider domains are automatically allowed during login flows. You do not need to add these to `allowed_domains`:
...
For other SSO providers, add their domains to `allowed_domains` when creating the connection.

## Error codes

When a login flow fails, `error_code` contains a machine-readable code for programmatic handling:

| Code | Description | Recoverable |
|------|-------------|-------------|
| `credentials_invalid` | The credentials were rejected by the website | Yes |
| `login_form_not_found` | No login form was found on the page | Yes |
| `bot_detected` | The website detected automated access | Yes |
| `captcha_blocked` | A CAPTCHA blocked the login attempt | Yes |
| `mfa_required` | MFA is required but couldn't be completed | Yes |
| `website_error` | The website displayed an error during login | Yes |
| `session_expired` | The login session timed out | Yes |
| `element_not_found` | A required page element wasn't found | Yes |
| `navigation_confused` | The login flow navigated to an unexpected page | Yes |
| `stuck_in_loop` | The login flow got stuck repeating the same action | Yes |
| `max_attempts_reached` | Maximum retry attempts were exhausted | No |
| `browser_error` | The browser encountered an error | No |
| `internal_error` | An internal error occurred | No |
