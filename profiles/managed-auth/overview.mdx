...
Under the hood, Managed Auth uses a Computer Use Agent (CUA) to navigate login pages. The agent discovers forms, fills credentials, handles multi-step flows, and completes authentication. Successful login flows are recorded as plans and replayed for faster subsequent logins.
...
- **Works on any website** - Login pages are discovered and handled automatically by the CUA
- **SSO/OAuth support** - "Sign in with Google/GitHub/Microsoft" buttons work out-of-the-box. Common SSO provider domains (Google, Microsoft, Okta, Auth0, GitHub, etc.) are auto-allowed.
...
- **Proxy support** - Route auth flows through a proxy for geo-restricted or bot-protected sites
- **Session monitoring** - Automatic re-authentication when sessions expire, with configurable health check intervals
