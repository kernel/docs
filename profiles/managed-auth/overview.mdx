...
- **Works on any website** - Login pages are discovered and handled automatically using AI vision
- **SSO/OAuth support** - "Sign in with Google/GitHub/Microsoft" buttons work out-of-the-box. Common SSO providers (Google, Microsoft, Okta, Auth0, and others) are automatically allowed — no need to configure `allowed_domains` for them
...
- **Website error detection** - Get real-time feedback when the website shows errors (e.g., "Incorrect password") via `website_error`
- **Post-login URL** - Get the URL where login landed (`post_login_url`) so you can start automations from the right page
- **Session monitoring** - Automatic re-authentication when sessions expire with stored credentials. Check `can_reauth` and `can_reauth_reason` to understand re-auth eligibility
- **Proxy support** - Route login flows through residential, ISP, or datacenter proxies
