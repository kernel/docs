...
- **Session monitoring** - Automatic re-authentication when sessions expire with stored credentials, with [configurable health check intervals](#health-check-interval)
- **Live view** - Watch the login flow in real-time via `live_view_url` for debugging and monitoring
- **Proxy support** - Route auth flows through [proxies](/proxies/overview) for geo-restricted sites
- **Secure by default** - Credentials encrypted at rest, never exposed in API responses, or passed to LLMs

## Health check interval

By default, Kernel checks authentication status every hour and re-authenticates if needed. You can configure this interval per connection:
