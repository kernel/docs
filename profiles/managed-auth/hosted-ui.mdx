...
Route auth flows through a [proxy](/proxies/overview) for geo-restricted sites or to avoid bot detection:
...
You can also pass a proxy when starting a login session to override the connection's default:
...
The following SSO/OAuth provider domains are automatically allowed and don't need to be added to `allowed_domains`:
...
For other providers, add their domains to `allowed_domains`:
...
const auth = await kernel.auth.connections.create({
  domain: 'example.com',
  profile_name: 'my-profile',
  allowed_domains: ['auth.custom-sso.com'],
});
```
...
The login response includes a `live_view_url` for watching the authentication flow in real-time. This is useful for debugging or monitoring:
...
Configure how frequently Kernel checks if the session is still authenticated. See [Health check interval](/profiles/managed-auth/overview#health-check-interval) for plan-specific minimums.
