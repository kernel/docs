...
Route the login flow through a proxy for sites that require residential or ISP IP addresses:
...
You can also override the proxy when starting a login session:
...
```python Python
login = await kernel.auth.connections.login(auth.id, proxy={"name": "different-proxy"})
```
</CodeGroup>

### Health check interval

Configure how often Kernel checks if the session is still authenticated. The interval is in seconds and must be between 300 (5 minutes) and 86400 (24 hours):
...
<Info>
The minimum health check interval depends on your plan: Hobbyist (1 hour), Start-Up (20 minutes), Enterprise (5 minutes). See [FAQ](/profiles/managed-auth/faq#how-often-are-health-checks-performed) for details.
</Info>

### Live view

The login response includes a `live_view_url` that lets you watch the login flow happening in real-time. This is useful for debugging failed logins or understanding how Kernel navigates a site's login page:
