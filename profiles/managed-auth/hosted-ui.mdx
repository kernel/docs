...
Start a Managed Auth Session to get the hosted login URL. The response also includes a `live_view_url` for watching the login flow in real-time.
...
Common SSO/OAuth providers are automatically allowed by default, so you don't need to configure `allowed_domains` for them:
...
Control how often Kernel checks if the session is still authenticated. The default is 3600 seconds (1 hour). Minimums depend on your plan:
...
Route login flows through a proxy for sites that require specific IP addresses or geolocation:
...
See [Proxies](/proxies/overview) for more on proxy configuration.
