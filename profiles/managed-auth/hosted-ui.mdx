...
Send the user to the hosted login page. The login response also includes a `live_view_url` for watching the CUA navigate the login flow in real-time:

<CodeGroup>
```typescript TypeScript
window.location.href = login.hosted_url;

// Optional: watch the login flow in real-time
...
You can also reference credentials from an external provider like 1Password:
...
const auth = await kernel.auth.connections.create({
  ...
  credential: { provider: 'my-1p', path: 'Personal/Example' },
});

// Or auto-lookup by domain
const auth2 = await kernel.auth.connections.create({
  domain: 'example.com',
  profile_name: 'my-profile',
...
# Or auto-lookup by domain
auth2 = await kernel.auth.connections.create(
    domain="example.com",
    profile_name="my-profile",
    credential={"provider": "my-1p", "auto": True},
)
...
Common SSO provider domains are auto-allowed (Google, Microsoft, Okta, Auth0, GitHub, Apple, Facebook, LinkedIn, Amazon Cognito, OneLogin, Ping Identity). For other providers, add their domains to `allowed_domains`:
...
const auth = await kernel.auth.connections.create({
  domain: 'example.com',
  profile_name: 'my-profile',
  allowed_domains: ['login.custom-idp.com'],
});
```
...
Route the auth flow through a proxy for geo-restricted or bot-protected sites:
...
Configure how often Kernel checks if the session is still authenticated. The interval is in seconds, with plan-dependent minimums (Enterprise: 300s, Start-Up: 1200s, Hobbyist: 3600s):
