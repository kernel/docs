...
Sites with "Sign in with Google/GitHub/Microsoft" are supported. The CUA agent clicks the SSO button, completes the OAuth flow with the provider, and the authenticated session is automatically saved to the Kernel profile.

Common SSO provider domains are automatically allowed by default, including:
- **Google**: accounts.google.com
- **Microsoft/Azure AD**: login.microsoftonline.com, login.live.com
- **Okta**: \*.okta.com, \*.oktapreview.com
- **Auth0**: \*.auth0.com and regional variants
- **GitHub**: github.com
...
The minimum interval depends on your plan: Enterprise (300s / 5 min), Startup (1200s / 20 min), Hobbyist (3600s / 1 hr). Maximum is 86400s (24 hours). Default is 3600s (1 hour).
