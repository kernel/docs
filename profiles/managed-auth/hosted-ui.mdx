...
You can also use `live_view_url` from the login response to watch the login flow in real-time for debugging:
...
Common SSO provider domains (Google, Microsoft, Okta, Auth0, GitHub, and more) are [allowed by default](/profiles/managed-auth/overview#default-sso-domains). For other providers, add their domains to `allowed_domains`:
...
Route the authentication flow through a proxy for geo-targeting or IP management:
...
You can also override the proxy for a specific login session:

<CodeGroup>
```typescript TypeScript
const login = await kernel.auth.connections.login(auth.id, {
  proxy: { proxy_id: 'proxy_xyz789' },
});
```
...
Configure how often Kernel checks if the session is still authenticated. The default is 1 hour:
...
Minimum intervals depend on your plan: Enterprise (5 minutes), Startup (20 minutes), Hobbyist (1 hour). Maximum is 24 hours.

### Error handling

When a login flow fails, check `error_code` and `error_message` for details:
...
if (state.flow_status === 'FAILED') {
  ...
  // e.g., "credentials_invalid - The credentials were rejected by the website"
}
```
...
See [Error Codes](/profiles/managed-auth/overview#error-codes) for the full list of error codes.
