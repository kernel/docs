...
if (state.flow_status === 'FAILED') {
  ...
  switch (state.error_code) {
    ...
    case 'domain_not_allowed':
      // Add the redirect domain to allowed_domains
      break;
    case 'mfa_required':
...
if state.flow_status == "FAILED":
    ...
    if state.error_code == "credentials_invalid":
        ...
        # Try again with a proxy or stealth mode
        pass
    elif state.error_code == "domain_not_allowed":
        # Add the redirect domain to allowed_domains
        pass
...
|------------|-------------|------------------|
| `credentials_invalid` | The credentials were rejected by the website | Re-enter credentials |
| `bot_detected` | The website detected automated access | Try with a [proxy](/proxies/overview) or different IP |
| `captcha_blocked` | A CAPTCHA challenge blocked the login | Try with a proxy |
| `login_form_not_found` | Couldn't find the login form on the page | Set `login_url` on the connection |
| `navigation_confused` | Unexpected navigation during login | Set `login_url` on the connection |
| `domain_not_allowed` | Login redirected to a domain not in `allowed_domains` | Update `allowed_domains` on the connection |
| `stuck_in_loop` | The login flow repeated the same action | Check `allowed_domains` and `login_url` |
| `max_attempts_reached` | Too many attempts without completing login | Verify credentials and retry |
| `mfa_required` | MFA is required but wasn't provided | Handle MFA via [programmatic flow](#handling-different-input-types) |
| `external_action_required` | An external action (push, SMS, security key) is needed | Prompt user to complete the action |
| `element_not_found` | A required page element wasn't found | The site layout may have changed — retry |
| `website_error` | The website returned an error page during login | Retry later |
| `session_expired` | The login session timed out | Start a new login session |
| `flow_failed` | The login flow failed for an unclassified reason | Retry or contact support |
| `browser_error` | Browser or connection error | Retry |
| `internal_error` | Internal Kernel error | Retry or contact support |
