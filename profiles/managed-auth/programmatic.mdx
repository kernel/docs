...
Submit the selected SSO button using `sso_button_selector`:
...
| `AWAITING_INPUT` | Waiting for field values, SSO selection, or MFA selection |
...
When a flow fails, the `error_code` field provides a machine-readable code:
...
| `domain_not_allowed` | Redirected to a domain not in `allowed_domains` |
...
The error response also includes a `recoverable` boolean indicating whether the error can be retried, and an optional `doc_url` linking to relevant documentation.
