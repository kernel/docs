...
| `type` | string | Input type: `text`, `email`, `password`, `tel`, `number`, `url`, `code`, `totp` |
...
[{ name: 'otp', type: 'code', label: 'Verification code', required: true }]
```
...
if (state.mfa_options?.length) {
  // Available types: sms, call, email, totp, push, password
  for (const opt of state.mfa_options) {
    console.log(`${opt.type}: ${opt.label} ${opt.target ?? ''}`);
  }
...
if state.mfa_options:
    ...
    for opt in state.mfa_options:
        print(f"{opt['type']}: {opt['label']} {opt.get('target', '')}")
...
Each MFA option includes a `type`, `label`, and optionally a `target` (masked phone/email, e.g., `***-***-5678`) and `description` (additional instructions from the site).

After selecting an MFA method, the flow continues. Poll for `discovered_fields` to submit the code, or handle external actions for push notifications.
...
When the target website displays an error (e.g., "Incorrect password"), the CUA captures it in `website_error`:
...
This is distinct from `error_message`, which indicates a terminal flow failure (e.g., max attempts reached, browser error). Check `error_code` for machine-readable failure reasons.

### Live view

The login response includes a `live_view_url` for watching the CUA navigate the login flow in real-time:
...
The `flow_type` field indicates the type of the current flow:

| Type | Description |
|------|-------------|
| `LOGIN` | User-initiated login |
| `REAUTH` | Automatic re-authentication triggered by a health check |
