...
The response includes a `live_view_url` for watching the login flow in real-time:
...
<Info>
Common SSO provider domains (Google, Microsoft, Okta, Auth0, GitHub, and others) are [automatically allowed](/profiles/managed-auth/hosted-ui#ssooauth-support). For other providers, add their domains to `allowed_domains` on the connection.
</Info>
