...
The default interval is 1 hour. You can configure it per connection with `health_check_interval` (in seconds). The minimum depends on your plan:
...
See [Health check interval](/profiles/managed-auth/overview#health-check-interval) for usage examples.
