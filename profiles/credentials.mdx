...
- **Connect an external provider** — Use credentials from 1Password or other external providers, referenced by path or auto-matched by domain
...
## Credential reference modes

When linking a credential to a connection, there are three reference modes:

| Mode | Example | Description |
|------|---------|-------------|
| Kernel credential | `{ name: "my-creds" }` | Reference a credential stored in Kernel |
| External provider (path) | `{ provider: "my-1p", path: "Vault/Item" }` | Reference a specific item in an external provider |
| External provider (auto) | `{ provider: "my-1p", auto: true }` | Auto-match by domain from an external provider |
