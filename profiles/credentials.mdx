...
When creating a connection, reference credentials using the `credential` parameter:

| Type | Example | Description |
|------|---------|-------------|
| Kernel credential | `{ name: 'my-creds' }` | Use a credential stored in Kernel |
| External provider (explicit) | `{ provider: 'my-1p', path: 'Vault/Item' }` | Use a specific item from an external provider |
| External provider (auto) | `{ provider: 'my-1p', auto: true }` | Search an external provider by domain |
