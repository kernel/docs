...
- Profiles store the full browser state, including cookies, local storage, and session data. When you create a new browser with a profile, it restores to the exact state when the profile was last saved—including open tabs and the current page.
- Start the session with `save_changes: true` to write changes back when the browser is closed.
