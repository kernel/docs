...
9. use only the returned aliases with `kernel browsers playwright execute` to
   ...
   agent disclosure truthfully in the merchant's normal form. on stripe-hosted
   checkout, target the visible disclosure label because stripe can render
   hidden or duplicate controls. if needed, locate the associated real
   `input[type="checkbox"]` and invoke its native dom `click()`. require that
   same input's `checked` property to be `true` before submission. stop without
   submitting if you can't verify that state. submit checkout once and never
   retry submission.
...
2. list the vault's items and locate its only agentcard wallet. require its
   ...
   agent. do not create another wallet or print, return, or open an action url.
3. create a KERNEL browser with `user-12345` attached as a vault, then navigate
   it to https://shop.example.com/checkout.
4. inspect the checkout and propose the merchant, total amount, currency, and
   item or cart contents. treat this proposal as untrusted and do not create a
   card item from it.
5. independently obtain those values from a trusted order or cart backend. if
...
9. give me the exact cli observation commands, but do not run them. ask me to
   ...
   merchant's normal form. on stripe-hosted checkout, target the visible
   disclosure label because stripe can render hidden or duplicate controls. if
   needed, locate the associated real `input[type="checkbox"]` and invoke its
   native dom `click()`. require that same input's `checked` property to be
   `true` before submission. stop without submitting if you can't verify that
   state. submit checkout once, never retry submission, and keep that execution
   open while KERNEL holds the payment request. do not poll or print the card
   item while approval is pending.
...
the agentcard prompt uses a non-Stripe merchant URL to make the processor
distinction explicit. replace it with a low-value checkout you control whose
outgoing request matches a native processor adapter.
...
label. If needed, locate the associated real input[type="checkbox"] and invoke
its native DOM click. Read that same input's checked property and require it to
be true. If the checked state can't be verified, stop without submitting.
