...
trusted backend, not the browser agent: see [link](/integrations/payments/stripe-link#before-you-start)
or [agentcard](/integrations/payments/agentcard#before-you-start).
...
imported link grant recovery requires an
[explicit cutover](/integrations/payments/stripe-link#before-you-start) to a new
wallet for future purchases. retain unresolved old items and select the new
wallet explicitly rather than applying the single-wallet lookup below.

for default setup, your payment settings ui must allow at most one wallet item
for each provider in a vault. list the vault's items before rendering provider
controls, group wallet items by `spec.provider`, and apply this flow:
