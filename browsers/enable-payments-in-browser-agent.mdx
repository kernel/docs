...
description: "connect wallets and complete browser checkouts"
---

you're building a browser agent. you can add payments without passing the card
number or cvc through your application's injection code or model context.

create a <span className="kernel-brand-name">KERNEL</span> vault and connect a [wallet integration](/integrations/wallets/overview) through its provider-hosted flow. attach the vault when you create a browser session, let your agent discover the payment form, and prepare a card item for the verified purchase.

use KERNEL's [`fill` api](/vaults/fill) to safely inject stored values without passing them through your application's injection code or model context. your agent proposes fields and selectors; your trusted controller authorizes their use and calls KERNEL with references, not raw card values. KERNEL returns per-field outcomes. agentcard uses aliases and provider-executed checkout instead of `fill`.

for the offering and shared lifecycle, see [payments on KERNEL](/browsers/payments). wallet integrations determine how payment material is supplied and approved, not which merchant processor you use.

<Warning>
  `fill` does not isolate card values from the browser. page scripts, extensions, and agents with browser or cdp access may read them. don't assume recordings exclude those values.
</Warning>

you can use either [link by stripe](/integrations/wallets/stripe-link) or
[agentcard](/integrations/wallets/agentcard). choose based on how the payment
...
11. inspect the checkout result, fill outcomes, and item events. fill is not
   payment success. do not retry a failed, timed-out, or indeterminate payment;
   report the existing attempt for reconciliation.
```
...
- use a low-value checkout you control. for link, verify its https merchant origin and field selectors. for agentcard, require a [native processor adapter](/integrations/wallets/overview#checkout-and-processor-coverage). the merchant processor doesn't need to be stripe.
- link card creation is live-only. agentcard mode comes from the integration's configured credential.
- for agentcard, keep an application-owned `AGENTCARD_MODE` setting and verify it against the credential's mode. customer-owned configs return `test_mode` (`true` means sandbox); for KERNEL-managed credentials, confirm the deployment's mode. fail closed if the mode is unknown or mismatched.

use KERNEL-managed credentials by default. optional client setup belongs in your
trusted backend, not the browser agent: see [link](/integrations/wallets/stripe-link#bring-your-own-link-oauth-client)
...
<Warning>
  ...
  action urls only in a trusted user-facing surface. for link, give the controller the item reference and approved field bindings, not raw card values. for agentcard, give the agent only the aliases returned in card state.
</Warning>
...
represents a provider-backed purchase and its authorization state; creating one
...
| your browser agent          | proposes purchase values and field selectors; after verification and confirmation, uses `fill` through the controller or enters agentcard aliases; submits once |
| KERNEL                      | creates vault resources, enforces browser bindings, uses `fill` to enter values, or intercepts and replays agentcard requests |
| the wallet provider     | hosts wallet connection, enrollment, and approval; stores or mints the underlying payment credential                                                  |
...
hide the add option for every provider that already has a wallet in the vault.
...
8. ask KERNEL to prepare a card item on the end user's behalf. link issues a single-use card for the approved purchase. agentcard can reuse a ready item and update its specification when the api permits. the item refers to the connected wallet; it is not the end user's real card.
...
10. wait for the card item to become `ready`. for link, require the advertised `fill` operation; for agentcard, pass its aliases to the browser agent.
...
<Note>
  we're adding wallet-provider integrations and merchant-processor adapters
  ...
  wallet providers and third-party vaults they already use. over time, we want{" "}
  <span className="kernel-brand-name">KERNEL</span> to support more of the
  browser-side steps required to complete a payment, with explicit exposure boundaries for each provider.
</Note>

both flows require a card item whose `state.status` is `ready`. link cards advertise `fill` when eligible and never expose `state.aliases`. agentcard cards expose aliases for `number`, `cvc`, `exp_month`, and `exp_year`.

### Collect non-card checkout fields

agentcard aliases cover only the card number, cvc, and expiry. `fill` also supports stored [billing fields](/integrations/wallets/stripe-link#map-card-fields-to-inputs), but requesting an absent billing value returns `field_unavailable` before any writes. merchant checkouts can also require email, shipping address, phone number, or other customer data. collect missing values from the end user in your trusted application, or load values the end user has already approved from your backend. pass ordinary customer data separately from payment input; don't ask the agent to invent it.
...
retrieve the item before checkout and recheck the advertised operation or aliases immediately before use. don't cache availability or agentcard aliases after an item expires, changes state, or is deleted.
...
## 4. Fill payment fields

<a id="link-invoke-fill" />

### Invoke KERNEL's fill api

use the same attached browser to inspect the checkout and invoke KERNEL's `fill` api. `checkoutURL` / `checkout_url` is its exact current top-level url, obtained through your browser-control method; no playwright page object is required. your trusted controller must authorize the destination and selectors before disclosure.

these examples use an approved link-issued card. its current https page must share the origin of `spec.merchant_url`; see [link card requirements](/integrations/wallets/stripe-link#fill-the-checkout) for eligibility and field formats. the `fill` request itself uses references and selectors, never raw card values.
...
retain each result's zero-based `index`, `status`, and optional `error_code`. `failed` may leave earlier writes in place; `unknown` or a transport error means the outcome is uncertain. stop without automatic retries or alias fallback. the clients in step 1 disable sdk retries. see [link checkout](/integrations/wallets/stripe-link#fill-the-checkout) for the cli equivalent, billing fields, and expiration formats, and the [fill outcome contract](/vaults/fill#handle-the-outcome) for per-field results.

a completed fill is not payment success. inspect the page, complete any other required fields and disclosures, and decide whether to submit checkout once. skip the agentcard observer below and continue to [verify the outcome](#6-verify-the-outcome). fill does not consume the link item or clear its encrypted material; provider-issued single-use card semantics are separate from item state.

### Agentcard: pass aliases

retrieve the card again and require agentcard aliases before starting the observer in step 5:
...
## 5. Agentcard only: observe checkout approval

this section applies only to agentcard. after using `fill`, continue to [verify the outcome](#6-verify-the-outcome). start observing the card item before the agent submits checkout and keep the observer running until the merchant reaches a terminal order state. the outgoing checkout request is held while the user approves it, then KERNEL replays the provider's response. link approval happens before fill; do not wait for an alias-substitution event on that path.
...
order. a completed `fill`, a ready agentcard item, an approved authorization, a delivered replay, or
