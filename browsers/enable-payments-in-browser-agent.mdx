...
use the same attached browser to inspect the checkout and fill it. `page` below is its top-level playwright page; the selectors must match the actual checkout. require the exact current https url, including query and fragment, at the origin of the card's `spec.merchant_url`. a different subdomain or non-default port isn't allowed. the card must remain ready, unexpired, and undeleted with stored material and an existing parent wallet.
