...
<Warning>
  KERNEL's [`fill` api](/vaults/fill) writes actual card values into the browser.
  your controller sends item references and approved selectors, not raw values.
  don't read filled values back into model context or assume recordings omit them.
</Warning>
