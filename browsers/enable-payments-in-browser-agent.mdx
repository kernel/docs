...
<Note>
  this recipe uses payment aliases resolved at egress. for direct filling of a
  ready link by stripe card, see [fill browser fields](/vaults/fill): real values enter the
  browser, unlike the alias path below. don't switch paths after an uncertain
  attempt. for passwords and other non-payment credentials, use the
  [credential cookbook](/browsers/use-vault-credentials-in-browser-agent).
</Note>
