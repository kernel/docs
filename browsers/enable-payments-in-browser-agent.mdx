...
8. retrieve the card item. invoke `authorize` only if it appears in
...
9. start a separate observer for the card item and its events. use only the
...
10. when the observer returns an approval action, use `--open` and pause while i
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| your application     | independently verifies purchase values; calls KERNEL on the end user's behalf; presents provider actions; observes state; attaches the vault |
