...
number or cvc to your application or agent.

create a <span className="kernel-brand-name">KERNEL</span> vault and add a wallet backed by [link](/integrations/payments/stripe-link) or [agentcard](/integrations/payments/agentcard). the provider-hosted flow collects and stores the user's payment method, so neither you nor your agent handles the card number. attach the vault when you create a browser session, let your agent discover the payment form and request a card to use. your agent is handed a tool to fill out the web checkout payment form and never sees the raw card details.

link and agentcard are wallet providers, not merchant payment processors. link fill requires supported inputs on the allowed merchant origin, not a processor adapter. agentcard's alias handoff requires the outgoing request to match a [native processor adapter](/integrations/payments/overview#checkout-and-processor-coverage).

<Warning>
  link fill does not isolate card values from the browser. page scripts, extensions, and agents with browser or cdp access may read them. don't assume recordings exclude those values.
</Warning>
...
- use a low-value checkout you control. for link, verify its https merchant origin and field selectors. for agentcard, require a [native processor adapter](/integrations/payments/overview#checkout-and-processor-coverage). the merchant processor doesn't need to be stripe.
...
trusted backend, not the browser agent: see [link](/integrations/payments/stripe-link#bring-your-own-link-oauth-client)
or [agentcard](/integrations/payments/agentcard#bring-your-own-agentcard-oauth-client).
...
## 4. Fill link cards or pass agentcard aliases

### Link: invoke fill

use the same attached browser to inspect the checkout and fill it. `page` below is its top-level playwright page; the selectors must match the actual checkout. require the exact current https url, including query and fragment, at the origin of the card's `spec.merchant_url`. the host and effective port must match the `merchant_url` origin; the default https port `443` is normalized. the card must remain ready, unexpired, and undeleted with stored material and an existing parent wallet.
...
## 5. Keep agentcard approval and observation outside the agent

this section is the agentcard path. start observing the card item before the agent submits checkout and keep the observer running until the merchant reaches a terminal order state. the outgoing checkout request is held while the user approves it, then KERNEL replays the provider's response. link approval happens before fill; do not wait for an alias-substitution event on that path.
...
| link fill `completed` or `credential_fill_attempted` | inspect per-field outcomes and merchant state; filling alone does not prove submission or payment success |
| link fill `failed`, `unknown`, or a lost response | preserve partial or uncertain outcomes; stop without automatic retries or alias fallback |
