...
description: "connect wallets and complete browser checkout with link fill or agentcard aliases"
---

create a <span className="kernel-brand-name">KERNEL</span> vault and connect a user's [link](/integrations/payments/stripe-link) or [agentcard](/integrations/payments/agentcard) wallet through its provider-hosted flow. attach the vault when you create the browser session.

link checkout uses the card item's advertised [fill](/vaults/fill) operation. it writes actual card values into the browser without returning them in the api response or explicitly submitting checkout. agentcard checkout uses aliases and processor interception, approval holds, and response replay outside the browser.
...
| checkout        | fill actual card values; no aliases or egress substitution        | enter aliases; KERNEL holds a recognized request for approval and replays the response |
| reuse           | provider-issued single-use card; fill does not consume the item   | card item and aliases return to `ready`; every checkout requires approval        |
...
| your browser agent          | proposes purchase values and field selectors; after verification and confirmation, uses link fill through the controller or enters agentcard aliases; submits once |
| KERNEL                      | creates vault resources, enforces browser bindings, fills link card values, or intercepts and replays agentcard requests |
...
order. a completed link fill, a ready agentcard item, an approved authorization, a delivered replay, or
