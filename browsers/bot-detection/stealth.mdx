...
1. **Default proxy** — traffic routes through a static residential ISP proxy, providing a stable exit IP that appears residential (due to its ISP ASN) with datacenter-level performance.
