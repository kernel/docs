...
<Info>
[Web Bot Auth (WBA)](/browsers/bot-detection/web-bot-auth) adds a verifiable signing identity that participating sites can recognize and allow. KERNEL's WBA token is a separate opt-in feature, enabled selectively on request for Startup Plan and Enterprise Plan customers. Enabling stealth mode alone doesn't opt you in. [Request WBA access](/browsers/bot-detection/web-bot-auth#request-wba-access).
</Info>
