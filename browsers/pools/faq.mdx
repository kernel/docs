...
`fill_rate_per_minute` is the percentage of the pool that will be refilled per minute, and defaults to 25. At the default, the pool refills 25% of its size per minute, so a pool of 20 warms up completely in about four minutes.
...
Yes — `kernel.browserPools.update()` changes the configuration used for browsers created after the update. Existing idle browsers keep their old config unless you pass `discard_all_idle: true` (or flush the pool), and in-use browsers are never touched mid-acquisition. See [Update a pool](/browsers/pools/overview#update-a-pool) for how to roll an in-use browser onto the new configuration.
