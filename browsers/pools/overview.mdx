...
## Why pre-configure browsers with browser pools

- Browser pools allows you to provision browsers with your specific configuration ahead of time (such as stealth, proxies, extensions, and profiles), rather than the setup latency happening at agent runtime. 
- Acquiring a browser from a pool returns a ready browser, faster.
- Reserved browsers aren't billed until they're used, so an idle pool of reserved browers costs nothing.

See [Scale](/introduction/scale) for how pools fit into best practices for production architecture.
