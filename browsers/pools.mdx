...
Browsers wait in the browser pool indefinitely until acquired — a browser pool's `timeout_seconds` (default 600 seconds, max 259200) only starts running once a browser is acquired. From there it behaves like a [regular browser timeout](/browsers/termination#automatic-deletion-via-timeout): if the browser sits idle, with no CDP or live view connection, for longer than the timeout, it's destroyed rather than returned to the pool, and the pool creates a replacement.
