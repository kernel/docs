...
Browser pools let you configure a set of identical browsers ready for immediate use. Set your configuration once on the browser pool — stealth, proxies, extensions, viewport, and a [profile](#profiles-with-browser-pools) — instead of configuring each on-demand browser you create. Every browser it hands you is ready to drive with those settings already applied.

Acquiring from a browser pool is faster than creating an on-demand browser: a pooled browser is already running, so you skip start-up and the [Chromium restart](/browsers/performance#troubleshooting-latency) that some settings trigger on a new browser, and you avoid the [rate limit](/info/pricing#rate-limiting) that applies to on-demand browser creation. Idle browsers in a browser pool aren't billed, though its capacity counts against your [concurrency limit](/info/pricing#concurrency-limits); see [Scale](/introduction/scale) for how browser pools fit into production architecture.
...
`retrieve` returns `available_count` (browsers ready to acquire right now) and `acquired_count` (browsers in use). Watch `available_count` to size the browser pool:
...
Browser pools shine at higher concurrency, but even a small one is worth it whenever acquisition latency matters and demand is steady enough to keep it utilized — for example a latency-sensitive, user-facing action where a cold start is unacceptable. On-demand browsers remain the better fit for low-volume, bursty, or one-off work, for per-session varying configuration, and for GPU browsers.
...
<Info>
The `size` parameter is always required when updating a browser pool, even if you only want to change other settings.
</Info>
...
`flush` destroys every idle browser in the browser pool and lets it refill with the current configuration. Browsers that are currently acquired are left alone, so it's safe to call under load — it just won't touch anything in use.

## API reference

See the [Browser Pools API reference](https://kernel.sh/docs/api-reference/browser-pools/list-browser-pools) for `retrieve`, `list`, `delete`, and the full parameters and response shapes of every endpoint.
