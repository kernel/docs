...
Capture and export are separate steps: a session captures events whether or not you export them. To send a session's events to Honeycomb, Datadog, Grafana Cloud, or any other OTLP/HTTP collector, create a destination once and reference it when you create the browser:
...
See [Export Telemetry](/browsers/telemetry/export) for destination setup, credential rotation, and the constraints on export.
...
- [Export telemetry](/browsers/telemetry/export) - ship captured events to your own OTLP backend.
