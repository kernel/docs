...
input and captured strings are capped at 8,192 bytes on a character boundary and end in `...[truncated]` when clipped. the captured input is a string, not an object, and clipped input need not be valid JSON. tool output isn't captured. the MCP server's compact telemetry view can omit fields whose JSON-encoded size exceeds 8 KiB; use a bounded raw read when you need an omitted field.
