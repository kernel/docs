...
| `captcha_challenge_result` | Visible challenge | The challenge reached its overall `solved`, `failure`, `timeout`, or `abandoned` outcome. Wait for this event when you need the challenge result. |

Task events are best-effort and aren't ordered: a start can arrive after its result, and either event can be absent. Use each result event's `duration_ms` as the authoritative duration rather than calculating it from event timestamps. Challenge duration covers every solver attempt from when the challenge appeared until its terminal outcome.
