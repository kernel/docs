...
HTTP `status: 200` doesn't prove that a tool succeeded. check `invocation_status`: `completed`, `canceled`, `error`, or `awaiting_submission`. `awaiting_submission` means a non-autosubmit form was populated but not submitted. HTTP `504` with `invocation_status: outcome_unknown` and `error_code: outcome_unknown` means the action may have happened. don't retry automatically, even if `invocation_id` is absent; inspect the page state first. HTTP failures without an observed semantic outcome have no `invocation_status`.
