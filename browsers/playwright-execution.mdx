...
- `webmcp` - Frozen browser-wide helpers for discovering and invoking [WebMCP tools](/browsers/webmcp)
...
First inspect `await webmcp.listTools()` to verify the tool's source and `input_schema`. The example below assumes your page registers a top-level `search_products` tool accepting a `query` string. It uses an existing session and client, as in the examples above. Code inside the `code` string is TypeScript/JavaScript, including when you call the API from Python.
...
const response = await kernel.browsers.playwright.execute(sessionId, {
  code: `
    ...
    const matches = tools.filter(tool =>
      ...
      tool.source.page_url === page.url().split('#')[0] &&
      tool.source.frame === null
    );
    if (matches.length !== 1) return { tools };
...
Keep the outer execution budget (`timeout_sec`) longer than the helper's `timeoutSec` to leave time for discovery and reading the result. Check `response.success` for execution failures and `invocation.status` for the tool's terminal status (`completed`, `canceled`, or `error`).
