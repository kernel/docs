...
Set `region` on the pool to place every browser in that region:
...
[Acquire and release browsers](/browsers/pools#acquire-a-browser) as usual. Acquired browsers inherit the pool's region; you don't pass `region` on acquire. You can also choose **Region** when creating a pool in the dashboard.

A pool can't span regions, and you can't change its region after creation. Create a separate pool for each region you need. Pool names must be unique within a project across all regions, so use names like `checkout-eu` and `checkout-sg`.
