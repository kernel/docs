...
Loading a profile this way restores all of its saved tabs, which can leave the browser with several tabs open at once. Pass [`start_url`](/api-reference/browsers/update-browser-session#body-start-url) in the same `update()` call to collapse those restored tabs down to a single tab at the page you specify.
