...
| --- | --- | --- |
| `KERNEL_CONNECTED` | `connected`, `capabilities` | Signaling is established. Frames are not necessarily rendering yet. |
...
<Info>
    `KERNEL_CONNECTION_TIMEOUT`, `KERNEL_READ_ONLY_CHANGED` and `KERNEL_SET_READ_ONLY` require a recent browser image. `KERNEL_CONNECTED`, `KERNEL_PLAYING` and `KERNEL_PAUSED` are available on all current images.
