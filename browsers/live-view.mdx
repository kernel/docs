...
<Tip>
    Interacting with an embedded third-party iframe like live view requires browser focus. On Safari, focus requires a user-initiated event — calling `.focus()` on the iframe element within a user-initiated event handler is recommended.
