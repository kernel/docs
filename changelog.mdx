...
<Update label="September 25" tags={["Product", "Docs"]}>
## Product updates

- Added **custom WebMCP tools**: register named groups of tools backed by the page or by CDP through the API or the [Browser REPL](/browsers/repl). They are discovered alongside a site's own WebMCP tools and called the same way. Manage them with `kernel browsers webmcp custom-tools list/add/remove` in the [CLI](https://github.com/kernel/cli), and call them from the [MCP server](/reference/mcp-server).
- Added **per-host proxy routes** at browser creation: send specific hosts through a chosen proxy with the repeatable `--proxy-route` flag in the CLI, or through the MCP server.
- Added **Search API** access through `kernel search`, `kernel search get`, and `kernel search providers` in the CLI, and a web search tool in the MCP server for accounts with Search enabled.
- Added **profile binding to browser pool acquire**, so a browser taken from a pool can start with a specific profile loaded.
- Added optional human-readable **labels** on [credential vault item](/vaults/credentials) fields. Stable field names still control updates and `fill`. Supported in the API, CLI, and MCP server.
- Added the persistent [Browser REPL](/browsers/repl) as an MCP server tool. MCP vault tools can now call any vault operation the API advertises, and they return the provider's reason when a card authorization is rejected.
- Improved `kernel mcp install`: added Google Antigravity as a target, gave each stdio client its own named OAuth identity and auth cache, and changed install to merge only the Kernel entry. Hand-edited client config is preserved and config files are written with private permissions.
- Released [`@onkernel/vault-react`](https://www.npmjs.com/package/@onkernel/vault-react) 0.2.0 with a show/hide control for sensitive fields, and added the same control to the Kernel-hosted [credential](/vaults/credentials) collection form. Stored secrets stay omitted; only newly entered values can be revealed.
- Paced [`fill`](/vaults/fill) writes character by character, with a short randomized delay between characters and between fields, so values enter the page at human typing speed.
- Added `iframe` and `worker` to the published browser target types. The [telemetry](/browsers/telemetry/overview) collector already attached to both, but the public enum omitted them.
- Added per-type filtering to the archived browser [telemetry](/browsers/telemetry/overview) events endpoint. It applies alongside the existing category filter, and an event must match both when both are set.
- Added current [concurrency usage](/info/projects#concurrency-limits) to the org limits API and `kernel org limits get`: `concurrent_sessions_used` and `concurrent_sessions_available` count active browser sessions and browser pool reservations against your organization limit.
- Added managed [proxy](/proxies/overview) selection to the dashboard's browser launch dialog for plans entitled to use them.
- Split the dashboard's managed auth reauthentication status into **Auto**, **Best effort**, and **Needs human**, so a connection that depends on an optimistic TOTP retry is no longer shown as fully automatic.
- Added playback speed control to session [replays](/browsers/replays).

## Documentation updates

- Added a [Browser REPL](/browsers/repl) guide, and rewrote the fx co-located agent cookbook to run `libfx` over the Browser REPL.
- Added a Google Antigravity MCP client page and a Replit integration guide. Documented named MCP OAuth clients and how to migrate their auth caches.
- Documented proxy error codes and the `X-Kernel-Proxy-Error` header, the parent-frame `postMessage` events the [live view](/browsers/live-view) exchanges with embedders — detecting a failed viewer, and toggling read-only mode without reloading the iframe — and computed telemetry readiness events.
- Documented which events and identifiers each `captcha_type` emits, and the challenge statuses each observed type reports. Challenge results are emitted for hCaptcha, press-and-hold, and reCAPTCHA v2; other types emit task events only.
- Documented credential field labels and how to check current concurrency usage.
- Added guidance on measuring session memory with [process execution](/browsers/process-execution) and on taking accessibility tree snapshots in the Playwright execution guide.
- Added a Jev browser use on Val Town cookbook, and documented the payment processors AgentCard supports.
- Added [`create-site-skills`](/skills/create-site-skills) to the skills docs, a Vault React collection starter example, and a `kernel-browser-loop` val link to the Val Town guide. Also corrected the Link amount limit.
</Update>

<Update label="September 18" tags={["Product", "Docs"]}>
## Product updates

- Extended [Vaults](/vaults/overview) beyond payments to general credentials: store login credentials as [vault items](/vaults/credentials), collect them from your end user through a Kernel-hosted form, and have Kernel [fill](/vaults/fill) them into the page without those values passing through your application code or your agent's context. Available through the API, the `kernel` CLI, and the MCP server.
- Extended [`fill`](/vaults/fill) to payment card items, so a [Link](/integrations/wallets/stripe-link#fill-the-checkout) checkout can be completed by injecting card fields the same way credentials are filled. [AgentCard](/integrations/wallets/agentcard) keeps its alias-based flow.
- Added [Vaults](/vaults/overview) to the dashboard: create project-scoped vaults, then create and inspect credential items alongside payment items. Proxy configuration also gained ISP country selection.
- Added `start_url` to browser session [`update()`](https://www.kernel.sh/docs/api-reference/browsers/update-browser-session). Loading a [profile](/browsers/profiles) restores every tab it saved; passing `start_url` in the same update collapses them onto a single page at the URL you specify.
- Added a persistent [Browser REPL](https://www.kernel.sh/docs/api-reference/browser-repl/execute-javascript-in-the-persistent-browser-repl) API: a long-lived Node.js runtime inside the browser VM, where top-level bindings, closures, and dynamically imported modules persist across calls. It exposes browser-control helpers, WebMCP, Patchright, Playwright, and raw CDP, and `kernel browsers repl` in the [CLI](https://github.com/kernel/cli) (v0.38.2) drives the same runtime from your terminal. Executions are serialized, and a timeout, crash, or protocol failure terminates the REPL and issues a new `repl_id`.

## Documentation updates

- Added cookbooks: a Jev browser use recipe, a [human-in-the-loop credentials walkthrough](/browsers/use-vault-credentials-in-browser-agent), [pausing an agent for captcha solves](/browsers/telemetry/pausing-for-captcha-solves), and an fx co-located agent that runs Vercel's `fx` coding agent inside the browser VM alongside the Chromium it drives.
- Added a [process execution](/browsers/process-execution) guide for running processes inside the browser VM.
- Reframed the [authentication](/auth/overview) docs around two integration paths and made browser [profiles](/browsers/profiles) first-class across guides.
- Documented [credential vault items](/vaults/credentials), the [`fill`](/vaults/fill) API and its browser-access boundary, and how to [copy values from an existing vault](/vaults/existing-credential-vault).
- Documented [browser payments](/browsers/payments) end to end: [enabling payments in a browser agent](/browsers/enable-payments-in-browser-agent), the [Link](/integrations/wallets/stripe-link) and [AgentCard](/integrations/wallets/agentcard) wallet integrations, optional payment provider clients, and Link's US phone number eligibility requirement.
- Documented that Playwright is no longer the only way to drive the page, including browser [WebMCP](/browsers/webmcp) support.
- Added a standalone [Regional Browsers](/browsers/regions) guide.
- Expanded proxy docs: regional ISP proxies, automatic `.gov` routing (and where it doesn't apply), removal of the residential-proxy creation-date restriction, and separate pricing rows for configurable vs. BYO proxies.
</Update>

<Update label="September 11" tags={["Product", "Docs"]}>
## Product updates

- Launched **payments for browser agents**: agents can now complete purchases without exposing card data to your application, agent, or browser. [Vaults](/vaults/overview) are a new primitive that grants a browser session permission to use a payment method without revealing the underlying credential. The agent fills the checkout form with aliases, and Kernel substitutes or authorizes the real credential at browser egress. Card numbers and CVCs stay out of the agent's context, your logs, and session recordings, which helps you avoid expanding your PCI DSS scope. Two native wallet integrations ship with it: [Link by Stripe](/integrations/wallets/stripe-link) for single-use credential substitution, and [AgentCard](/integrations/wallets/agentcard) for authorized request handoff. The merchant keeps its existing checkout and does not integrate with Kernel.
...
- Documented [Vaults](/vaults/overview) and [browser payments](/integrations/wallets/overview), including checkout and payment-processor support, app-invocation billing, and Vault payment security guidance, and added them to the [pricing page](/info/pricing).
...
- Moved OTLP telemetry destinations from project scope to organization scope and added dashboard management with a launch-time destination picker and per-session detail, making it easier to route [browser telemetry](/browsers/telemetry/overview) to your own observability stack.
...
- The [MCP server](/reference/mcp-server) now accepts a browser session name anywhere a session ID is expected, so you can reference browsers by the names you already use.

## Documentation updates

- Added a cookbook for [Playwright with a computer-use fallback](/browsers/playwright-computer-use-fallback) for steps pure Playwright can't complete.
- Documented the [live view domains](/browsers/live-view) the embedded live view loads from, so you can allowlist them.
- Documented captcha task and challenge [telemetry categories](/browsers/telemetry/categories), including when events carry a `challenge_id`.
- Refreshed the [managed auth docs](/auth/connection-lifecycle) — canonical interactions, typed reauth outcomes, and the authenticator-requirement outcome — and improved managed auth discoverability across the docs.
- Documented how to [connect the MCP server with an API key](/reference/mcp-server/troubleshooting) when a client's strict OAuth issuer verification rejects the flow.
</Update>
...
- Added [safe profile writes](/browsers/profiles/concurrency) and clarified the `save_changes` requirement for [per-user pool profiles](/browsers/pools).
...
- Documented [custom proxy CA bundles](/proxies/custom) for BYO MITM setups, [audit log search and export](/info/audit-logs), [per-user profile persistence through browser pools](/browsers/pools/overview), and [managed auth tab ownership](/browsers/profiles/save-and-reuse#control-the-starting-tab).
...
- Deprecated the "persistent" feature from browsers in favor of using extended `timeout_seconds` with [Profiles](/browsers/profiles/save-and-reuse#3-use-and-delete-the-browser); added `name` field to browsers for easier identification
