...
- Improved managed auth reliability and feedback: [`@onkernel/managed-auth-react`](https://github.com/kernel/managed-auth-react) now renders typed errors, verification returns authoritative tri-state outcomes, and reauth eligibility is risk-aware. The login agent also accounts for every visible field and authentication choice on the current auth surface — including controls inside nested frames — before pausing for input.
