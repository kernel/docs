...
- Added automatic action payload type capture when deploying apps, enabling better autocomplete and validation in the dashboard playground and MCP server.
...
- Streamlined the [app deployment](/apps/deploy) experience by prefetching action payload types, allowing users to see clean forms with labeled inputs instead of manually constructing JSON payloads.
