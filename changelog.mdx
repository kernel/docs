...
- Reduced live view scroll sensitivity for smoother, more precise scrolling during browser sessions.
...
- Resolved an issue where browser pool acquisition could be slow when the selected pool was exhausted, even when other pools had idle sessions available.
...
- Improved mouse movement in live view by resolving a VP9 video encoding configuration issue.
- Resolved network requests hanging when routed through a proxy.

## Documentation updates

- Added a FAQ entry for [debugging managed auth sessions](/auth/overview).
- Updated [Managed Auth](/auth/overview) documentation to cover CUA support, the PATCH endpoint, and auto-allowed SSO domains.
- Added notes about [GPU-accelerated browsers](/browsers/gpu-acceleration) and standby mode compatibility to the [browser pools](/browsers/pools) and [scaling](/browsers/scaling/overview) docs.
</Update>
...
- Added support for [runtime proxy configuration swapping](https://www.kernel.sh/docs/proxies/overview#5-update-a-browser%E2%80%99s-proxy), enabling you to change a browser's proxy settings mid-session without restarting.
