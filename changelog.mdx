...
- Documented [restricted-destination routing](/proxies/residential#restricted-destination-routing) in full. Residential proxies with no targeting, country-only targeting, or US state targeting route `.gov`, country government domains, and a set of banking and payment destinations through a provider that carries them. The docs previously described this as `.gov`-only and said banking domains always fail at the proxy layer.
