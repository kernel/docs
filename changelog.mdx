...
- Added a persistent [Browser REPL](/api-reference/browser-repl/execute-javascript-in-the-persistent-browser-repl) API: a long-lived Node.js runtime inside the browser VM, where top-level bindings, closures, and dynamically imported modules persist across calls. It exposes browser-control helpers, WebMCP, Patchright, Playwright, and raw CDP, and `kernel browsers repl` drives the same runtime from your terminal. Executions are serialized, and a timeout, crash, or protocol failure terminates the REPL and issues a new `repl_id`.
- Launched the **Config Registry**: Kernel resolves and stack-ranks working browser and proxy configurations for a target site and returns site-specific guidance, available through the [API](/api-reference/config-registry/look-up-a-config), the [MCP server](/reference/mcp-server), and the dashboard. See the [Config Registry](/config-registry) guide.
...
- Added `start_url` to browser session [`update()`](/api-reference/browsers/update-browser-session). Loading a [profile](/browsers/profiles) restores every tab it saved; passing `start_url` in the same update collapses them onto a single page at the URL you specify.
- [`GET /org/limits`](/api-reference/organization/get-organization-limits) now reports `concurrent_sessions_used` and `concurrent_sessions_available`, so you can check how much browser concurrency your organization has free before launching sessions.
...
- Added a [Config Registry](/config-registry) guide and a [process execution](/browsers/process-execution) guide for running processes inside the browser VM.
- Reframed the [authentication](/auth) docs around two integration paths and made browser [profiles](/browsers/profiles) first-class across guides.
