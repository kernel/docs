...
<Update label="September 22" tags={["Docs"]}>
## Documentation updates

- Documented [restricted-destination routing](/proxies/residential#restricted-destination-routing) in full. Residential proxies route `.gov`, country government domains, and a set of banking, payment, and postal destinations through a provider that carries them, and the page now covers which targeting configurations qualify and how the routing affects a session's exit IPs. The docs previously described this as `.gov`-only and said banking domains always fail at the proxy layer.
- Documented that [ISP](/proxies/isp#restricted-destinations) and [mobile](/proxies/mobile#restricted-destinations) proxies don't route these destinations automatically, and what to use instead.
</Update>
