...
This only changes the key **your own API calls** authenticate as. Kernel still injects the deployment key and uses it to run the invocation and report its result, so you can't replace the deployment key — you're only choosing the credential for the calls your app makes.
